...
$setupOutput = Join-Path $workDirectory 'MajesticBoost-Setup-1.3.0.exe'
...
Copy-Item -LiteralPath $setupOutput -Destination (Join-Path $distDirectory 'MajesticBoost-Setup-1.3.0.exe') -Force

Get-FileHash -Algorithm SHA256 -LiteralPath @(
    (Join-Path $distDirectory 'MajesticBoost.exe'),
    (Join-Path $distDirectory 'MajesticBoost-Setup-1.3.0.exe')
